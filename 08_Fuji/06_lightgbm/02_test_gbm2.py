...
# 4. 目的変数の逆変換フック
# =========================================================
def inverse_transform_target(y_array: np.ndarray, target_name: str):
    """
    今回は何も変換していないのでそのまま返す。
    将来、標準化したらここで元に戻す。
    例:
        return y_array * y_std[target_name] + y_mean[target_name]
    """
    return y_array


# =========================================================
# 5. LightGBM + SHAP + 評価CSV
# =========================================================
def run_lgbm_with_shap():
    ...
    for tgt in TARGET_COLS:
        ...
        for fold_id, (tr_idx, te_idx) in enumerate(gkf.split(X, y_t, groups)):
            ...
            # ここで「元の次元」に戻す。今回はそのまま。
            y_te_inv = inverse_transform_target(y_te, tgt)
            y_pred_inv = inverse_transform_target(y_pred, tgt)

            mae = mean_absolute_error(y_te_inv, y_pred_inv)
            rmse = mean_squared_error(y_te_inv, y_pred_inv, squared=False)
            ...
            eval_records.append({
                ...
                "mae": mae,
                "rmse": rmse,
                "y_true_mean": float(np.mean(y_te_inv)),
                "y_pred_mean": float(np.mean(y_pred_inv)),
            })
    ...
    # すべて終わったら評価結果を1つのCSVにする
    eval_df = pd.DataFrame(eval_records)
    eval_df.to_csv(ROOT / "cv_eval_results.csv", index=False)

    print("\nAll targets finished.")
    print(f"Saved evaluation to {ROOT / 'cv_eval_results.csv'}")
