...
"""
MultiOutputRegressor(SVR(kernel="rbf")) を GroupKFold(=Time[h]で4分割) で回す
~カーネルSVMを12出力にマルチタスクでかぶせた、非線形・黒箱寄りのモデル~
...
3. delta_30s
    定義：最大値とベースラインの差
4. auc_30s
    定義：30 s以降の波形の下面積(Area Under the Curve)
    連続では積分ですが、実際には0.1 s刻みなどの離散データなので、数値積分（台形公式）で求めました。
5. slope_30s
    定義：30〜40 sの間だけを取り出して、その区間に一次回帰を当て、傾きだけを取っています。
"""
...
# =========================================================
def train_and_eval(run_permutation: bool = False):
    # 1) データ構築（拡張）
    X, y, groups, tags = build_augmented_dataset()

    # 欠損を一括で埋める（本来は列ごとにやるのが望ましい）
    ...
    for t in TIMES:
        mask = (groups == t)
        # 同じ時間に属する予測を平均して1行に
        y_pred_mean = y_pred[mask].mean(axis=0, keepdims=True)   # (1, 12)
        # 同じ時間の真値は全部同じなので先頭を取る
        y_true = y[mask].iloc[0:1].to_numpy()                    # (1, 12)
        mae = mean_absolute_error(y_true, y_pred_mean)
        print(f"time {t}h: MAE(group-mean)={mae:.3f}, n_rows={mask.sum()}")
        group_rows_true.append(y_true)
        group_rows_pred.append(y_pred_mean)

    # 4点でのR²
    y_true_all = np.vstack(group_rows_true)   # (4, 12)
    y_pred_all = np.vstack(group_rows_pred)   # (4, 12)
    r2_all = r2_score(y_true_all, y_pred_all, multioutput="variance_weighted")
    print(f"\nR2 over 4 time-points (group-mean): {r2_all:.3f}")

    # 6) 予測の保存（行ごとのものも残す）
    out_pred = BASE / "01_appraisers_pred_svm.csv"
    ...
    # 7) ここから先はオプション: Permutation Importance
    if run_permutation:
        print("\n[Permutation importance per target (group-aware)]")
        # ベストモデルを取り出す
        best_model = gscv.best_estimator_
        scaler = best_model.named_steps["scaler"]
        mor = best_model.named_steps["svm"]  # MultiOutputRegressor

        # 各ターゲットごとに、元の「拡張済みX/y」でPIを回す。
        # GroupKFoldで評価したい場合は自前でsplitして平均してください。
        for j, target in enumerate(TARGET_COLS):
            # 単一ターゲットのパイプラインを組み直す
            single_pipe = Pipeline([
                ("scaler", scaler),
                ("svr", mor.estimators_[j])
            ])
            # このsingle_pipeはすでに学習済み（refit後）なので、
            # そのままPIを計算できる
            ...
            # 必要ならCSVにも保存
            imp_out = BASE / f"pi_{target}.csv"
            imp_df.to_csv(imp_out, index=False)
            print(f"saved PI: {imp_out}")


if __name__ == "__main__":
    train_and_eval(run_permutation=True)
