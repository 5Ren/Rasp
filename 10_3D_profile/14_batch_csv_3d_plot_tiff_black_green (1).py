#!/usr/bin/env python3
"""CSV height data -> black-background, green-axis 3D TIFFs.
...
# 参照画像と同様に、軸線と目盛線を同じ線幅で描く。
AXIS_LINE_WIDTH_PT = 0.5
AXIS_ARROWHEAD_SCALE_PT = 8.0
AXIS_XY_TICK_COUNT = 5
AXIS_Z_TICK_COUNT = 6
...
def add_arrow_axes(
    ...
    *,
    ...
    def segment_key(
        ...
        """同じ線分の重複描画を防ぐため、端点順に依存しないキーを作る。"""

        rounded_start = tuple(round(value, 12) for value in startpoint)
        rounded_end = tuple(round(value, 12) for value in endpoint)
        return tuple(sorted((rounded_start, rounded_end)))  # type: ignore[return-value]

    # 矢印軸と重なる外枠線は描かず、すべての線を実質0.5 ptにそろえる。
    drawn_segments = {
        segment_key(startpoint, endpoint)
        for startpoint, endpoint in arrow_segments
    }

    def draw_grid_segment(
        startpoint: tuple[float, float, float],
        endpoint: tuple[float, float, float],
    ) -> None:
        """黒色0.5 ptの外枠または目盛線を1本描く。"""

        key = segment_key(startpoint, endpoint)
    ...
    if draw_grid:
        x_ticks = np.linspace(0.0, x_max, AXIS_XY_TICK_COUNT)
        y_ticks = np.linspace(0.0, y_max, AXIS_XY_TICK_COUNT)
        z_ticks = np.linspace(z_base, z_top, AXIS_Z_TICK_COUNT)

        # 底面のX-Y目盛線。表面に隠れる部分を含めて面全体へ通す。
        for x_tick in x_ticks:
            draw_grid_segment(
                (float(x_tick), 0.0, z_base),
                (float(x_tick), y_max, z_base),
            )
        for y_tick in y_ticks:
            draw_grid_segment(
                (0.0, float(y_tick), z_base),
                (x_max, float(y_tick), z_base),
            )
        ...
        for x_tick in x_ticks:
            draw_grid_segment(
                (float(x_tick), y_max, z_base),
                (float(x_tick), y_max, z_top),
            )
        for y_tick in y_ticks:
            draw_grid_segment(
                (0.0, float(y_tick), z_base),
                (0.0, float(y_tick), z_top),
            )

        # Z方向の高さ目盛線。外枠を含み、元画像と同じ箱型の表示にする。
        for z_tick in z_ticks:
            draw_grid_segment(
                (0.0, y_max, float(z_tick)),
                (x_max, y_max, float(z_tick)),
            )
            draw_grid_segment(
                (0.0, 0.0, float(z_tick)),
                (0.0, y_max, float(z_tick)),
            )

    if draw_arrows:
        # 視点上の右端に位置するZ方向の外周線は、表面端に隠れないよう
        # 表面の後から描く。線幅と色はほかの軸線・目盛線と同一にする。
        right_z_axis = Arrow3D(
            (x_max, y_max, z_base),
            (x_max, y_max, z_top),
            arrowstyle="-",
            color=AXIS_COLOR,
        ...
        for startpoint, endpoint in arrow_segments:
            arrow = Arrow3D(
                ...
                endpoint,
                arrowstyle="-|>",
                mutation_scale=AXIS_ARROWHEAD_SCALE_PT,
                linewidth=AXIS_LINE_WIDTH_PT,
                color=AXIS_COLOR,
                shrinkA=0.0,
                shrinkB=0.0,
...
def save_3d_tiff(height: np.ndarray, output_path: Path) -> None:
    ...
    figure = plt.figure(figsize=figure_size_inches, facecolor=BACKGROUND_COLOR)
