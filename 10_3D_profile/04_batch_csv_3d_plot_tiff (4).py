...
# 色レンジの上下に余裕を持たせ、実データを両端色へ張り付かせない。
COLOR_RANGE_MARGIN_FRACTION = 0.05

# すべての画像で視点を固定する。
VIEW_ELEVATION_DEG = 24.0
VIEW_AZIMUTH_DEG = -62.0

# レーザー顕微鏡の高さ表示に近い、濃青 -> 水色 -> 緑 -> 黄 -> 赤の配色。
COLORMAP_NAME = "jet"  # 最小値: 濃い青、最大値: 濃い赤
...
def save_3d_tiff(height: np.ndarray, output_path: Path) -> None:
    ...
    if np.isclose(z_min, z_max):
        ...
        color_margin = (z_max - z_min) * COLOR_RANGE_MARGIN_FRACTION
        norm = Normalize(vmin=z_min - color_margin, vmax=z_max + color_margin)
