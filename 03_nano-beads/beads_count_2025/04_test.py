...
for filename in os.listdir(directory_path):
    if filename.lower().endswith(".bmp"):
        ...
        # ★ ここで「オリジナル画像上の正方形の一辺（ピクセル）」を自動決定する
        # 横 3 分割、縦 2 分割でちょうど入る最大サイズ
        square_side_length_pix_original = int(
            min(img_width / 3.0, img_height / 2.0)
        )

        # そのときの物理長さ [nm]（一応ログ用）
        square_side_length_nm = square_side_length_pix_original * scale_factor_nm_per_pix
        print(f"{filename}: square side ≒ {square_side_length_nm:.1f} nm")
        ...
        with open(csv_filename, 'w', newline='') as file:
            ...
            for j, (square_number, center, radius_display) in enumerate(circles):
                # リサイズ後の半径 → オリジナル画像の半径に戻す
                radius_original = radius_display / scale

                # nm への変換: diameter_nm = 2 * radius_original * [nm/pix]
                diameter_nm = 2.0 * radius_original * scale_factor_nm_per_pix
